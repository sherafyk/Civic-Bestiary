...
final class ACB_Shortcodes
{
    ...
    {
        add_action('init', array($this, 'ensure_cookie'));
    ...
    public function ensure_cookie()
    {
        ...
        if (!headers_sent()) {
            setcookie(self::COOKIE_NAME, $token, time() + YEAR_IN_SECONDS, COOKIEPATH ?: '/', COOKIE_DOMAIN, is_ssl(), true);
        }
    }
    ...
    public function render_assessment($atts = array())
    {
        ...
        } elseif (empty($profile['latest_result']) && (int) $profile['total_answered'] > 0) {
            $profile = $this->score_and_store($profile);
        }

        $questions = $this->repository->get_active_questions($limit, (int) $profile['id']);
        $dashboard_html = '';
        if ('yes' === strtolower((string) $atts['show_dashboard']) && !empty($profile['latest_result'])) {
        ...
        $empty_message = '';
        if ($questions) {
            $form_html = $this->render_form($profile, $questions, $settings);
        } elseif ((int) $this->repository->count_questions(true) > 0) {
            $empty_message = __('You have answered every active Bestiary question currently available.', 'american-civic-bestiary');
        } else {
    ...
    private function handle_submission(array $profile)
    {
        ...
        if (!$prepared) {
            return $this->notice(__('No new answers were saved.', 'american-civic-bestiary'), 'error');
        }
    ...
    private function render_form(array $profile, array $questions, array $settings)
    {
        $question_html = '';
    ...
    private function render_dashboard(array $profile, array $settings)
    {
        ...
        $top_matches = array_slice((array) ($result['animals']['matches'] ?? array()), 0, max(3, (int) ($settings['top_match_count'] ?? 8)));
        ...
        return ACB_Template::render('dashboard.php', array(
            ...
            'animal_bars' => $this->animal_bars($top_matches),
    ...
    private function animal_bars(array $rows)
    {
        return ACB_Template::render('components/bar-list.php', array(
            'rows' => $rows,
